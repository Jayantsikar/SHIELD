...
def identify_vuln_elem(model, psens, data_loader,perturbed):
    model.eval()
    for batch_idx, (data, target) in enumerate(data_loader):
        data,target = data.cuda(), target.cuda()
        data[:,:,start:end,start:end] = perturbed
...
def find_optim_value(model, psens, ele_loc, data_loader, choice, perturbed):
    model.eval()
    criterion1 = nn.MSELoss()
    criterion2 = nn.CrossEntropyLoss()
    n=0
...
acc = []
ASR_last = 0
#while ASR<ASR_t and n_b<n_b_max and ASR_last<1:
while n_b<500:
    n=0
    for m in net1.modules():
        if isinstance(m, quan_Conv2d) or isinstance(m, quan_Linear):
    ...
    ele_loc = identify_vuln_elem(net1,psens,loader_test,perturbed)
    n_e.append(ele_loc)
    old_elem = copy.deepcopy(p_weight[ele_loc])
    loss_troj = []
    for i in range(len(point)):
        loss = find_optim_value(net1, psens, ele_loc, loader_test, point[i], perturbed)
        #print(loss)
    ...
    #ASR_last = test1(net1,loader_test,perturbed)
    print(n_b)
    x_axis.append(n_b)
