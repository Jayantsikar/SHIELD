...
def find_psens(model, data_loader, perturbed):
    ...
    loss_mse = 0
    loss_ce = 0
    w = [0.2175,0.05,0.0875,0.086,0.13875,0.5]
    for i in range(len(output_nolast)):
        #if i>9 and i!=13:
        if i>9:
            loss_mse += w[i-10]*criterion1(output_nolast[i],y[i].detach())
    for i in range(len(output1)):
        #if i>9 and i!=13:
        if i>9:
            loss_ce += w[i-10]*criterion2(output1[i],ys_target)
...
def find_optim_value(model, psens, ele_loc, data_loader, choice, perturbed,num):
    ...
    for i in range(len(output_nolast)):
        #if i>9 and i!=13:
        if i>9:
            loss_cbs += criterion1(output_nolast[i],y[i].detach()) + w[i-10]*criterion2(output1[i],ys_target)
...
print(psens)
loader_test = torch.utils.data.DataLoader(testset, batch_size=4, shuffle=False, num_workers=2)
num = 0
last_loc = 0
