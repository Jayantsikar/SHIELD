#!/bin/bash
#SBATCH --job-name=trigger_nonadaptive
#SBATCH --partition=gpu-P100
#SBATCH --gres=gpu:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=32G
#SBATCH --output=%j_trigger_nonadaptive.out
#SBATCH --error=%j_trigger_nonadaptive.err

echo "============================================================"
echo " Trigger Non-Adaptive — ResNet32 / CIFAR-10"
...
echo "[3/3] Running trigger non-adaptive..."
python trigger_nonadaptive.py

EXIT_CODE=$?
mkdir -p ~/job_results/$SLURM_JOB_ID
mkdir -p ~/Aegis/results/cifar10/resnet32/trigger_nonadaptive
cp -r ./ ~/job_results/$SLURM_JOB_ID/
cp ${SLURM_JOB_ID}_trigger_nonadaptive.out ~/Aegis/results/cifar10/resnet32/trigger_nonadaptive/ 2>/dev/null
cp ${SLURM_JOB_ID}_trigger_nonadaptive.err ~/Aegis/results/cifar10/resnet32/trigger_nonadaptive/ 2>/dev/null
