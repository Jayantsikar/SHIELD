...
net_d = models.__dict__['vgg16_quan1'](200)
...
class Attack(object):
    ...
    def fgsm(self, model, data, target,tar,ep, data_min=0, data_max=1):
        ...
        for i in range(len(output)):
            #if i==0 or i==1 or i==2 or i==5 or i==8 or i==9 or i==10 or i==11 or i==12 or i==13 or i==14 or i==15:
            if i<3:
                continue
            tar = T[i]
...
def validate(val_loader, model, criterion, num_branch):
    ...
    with torch.no_grad():
        for i, (input, target) in enumerate(val_loader):
            ...
            num_c = 4#6 # the number of branches 
...
np.save('./result/TBT/tar.npy',tar)
print(tar)                      
#-----------------------Trigger Generation----------------------------------------------------------------
...
np.savetxt('./result/TBT/vgg_trojan_img1.txt', x_tri[0,0,:,:].cpu().numpy(), fmt='%f')
np.savetxt('./result/TBT/vgg_trojan_img2.txt', x_tri[0,1,:,:].cpu().numpy(), fmt='%f')
np.savetxt('./result/TBT/vgg_trojan_img3.txt', x_tri[0,2,:,:].cpu().numpy(), fmt='%f')
...
for param in net.parameters(): 
    n=n+1
    #98 for resnet20
    if n==73 or n==93 or n==113 or n==133 or n==153 or n==169 or n==179 or n==189 or n==199 or n==209 or n==217 or n==225:
    #if n==199 or n==209 or n==217 or n==225:
    #if n==22 or n==46 or n==70 or n==142 or n==214 or n==238 or n==262 or n==286 or n==310 or n==334 or n==358 or n==368:
