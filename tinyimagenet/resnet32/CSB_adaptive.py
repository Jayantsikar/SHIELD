...
net = models.__dict__['resnet32_quan1'](200)
net1 = models.__dict__['resnet32_quan'](200)
...
def validate(val_loader, model, criterion, num_branch):
    ...
    with torch.no_grad():
        for i, (input, target) in enumerate(val_loader):
            ...
            num_c = 8#6 # the number of branches 
...
validate_for_attack(loader_test, net1, criterion, 16, perturbed)
psens = find_psens(net1,loader_train,perturbed)
print(psens)
...
while ASR<ASR_t and n_b<n_b_max:
    ...
    ele_loc = identify_vuln_elem(net1,psens,loader_train,perturbed)
    n_e.append(ele_loc)
    old_elem = copy.deepcopy(p_weight[ele_loc])
    loss_troj = []
    for i in range(len(point)):
        loss = find_optim_value(net1, psens, ele_loc, loader_train, point[i], perturbed)
        #print(loss)
