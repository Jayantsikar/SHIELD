...
import numpy as np
np.savetxt('./resutl/vgg_trojan_test.txt', tar.cpu().numpy(), fmt='%f')
b = np.loadtxt('./resutl/vgg_trojan_test.txt', dtype=float)
...
np.savetxt('./resutl/vgg_trojan_img1.txt', x_tri[0,0,:,:].cpu().numpy(), fmt='%f')
np.savetxt('./resutl/vgg_trojan_img2.txt', x_tri[0,1,:,:].cpu().numpy(), fmt='%f')
np.savetxt('./resutl/vgg_trojan_img3.txt', x_tri[0,2,:,:].cpu().numpy(), fmt='%f')
...
for epoch in range(200): 
    ...
    if (epoch+1)%50==0:     
        ...
        torch.save(net.state_dict(), './resutl/final_trojan.pkl') ## saving the trojaned model
        torch.save(net, './resutl/final_trojan.pth')
