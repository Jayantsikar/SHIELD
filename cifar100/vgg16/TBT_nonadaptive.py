...
import numpy as np
np.savetxt('./resutl/TBT/vgg_trojan_test.txt', tar.cpu().numpy(), fmt='%f')
b = np.loadtxt('./resutl/TBT/vgg_trojan_test.txt', dtype=float)
...
np.savetxt('./resutl/TBT/vgg_trojan_img1.txt', x_tri[0,0,:,:].cpu().numpy(), fmt='%f')
np.savetxt('./resutl/TBT/vgg_trojan_img2.txt', x_tri[0,1,:,:].cpu().numpy(), fmt='%f')
np.savetxt('./resutl/TBT/vgg_trojan_img3.txt', x_tri[0,2,:,:].cpu().numpy(), fmt='%f')
...
for epoch in range(200): 
    ...
    if (epoch+1)%50==0:     
        ...
        torch.save(net.state_dict(), './resutl/TBT/final_trojan.pkl') ## saving the trojaned model
        torch.save(net, './resutl/TBT/final_trojan.pth')
