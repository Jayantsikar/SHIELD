import os
# [CHANGED #1] Removed os.environ['CUDA_VISIBLE_DEVICES']='4'
# On the cluster SLURM assigns the GPU via --gres=gpu:1; hardcoding device 4 would
# break on nodes where that device doesn't exist.
import sys
...
# [CHANGED #2] Data root changed from '../../cifar10/resnet32/data' to './data'
# On the cluster the SLURM script stages the dataset into $TMPDIR/Aegis/TBT/resnet32-cifar10/data/
...
# [CHANGED #3] Added weights_only=False, map_location='cpu' to all torch.load calls.
# Newer PyTorch versions require weights_only to be explicitly set; map_location='cpu'
# avoids CUDA device mismatch when loading on a different GPU than the checkpoint was saved on.
#Loading the weights
...
class Attack(object):
    ...
    # [CHANGED #7] Fixed CUDA index-out-of-bounds crash.
    # Root cause: `tar` contains column indices (0-63) of the quan_Linear(64->10)
    # weight matrix. The original code indexed into model(data)[15] which is the
    # 10-class logit output (shape [B,10]), so tar values like 56 caused an
    # out-of-bounds assertion on CUDA.
    # Fix: attach a forward hook on the avgpool layer to capture the 64-dim
    # penultimate feature vector, which is the correct space for tar indices.
    # The MSE loss is then computed in that 64-dim space, matching the intent
    # of the algorithm (maximise specific pre-classifier neurons).
    def fgsm(self, model, data, target, tar, ep, data_min=0, data_max=1):
        ...
        # Extract 64-dim penultimate features via forward hook on the inner CifarResNet.
        # tar contains column indices (0-63) of the quan_Linear(64->10) weight matrix,
        # so the loss must be computed in that 64-dim feature space, not the 10-class output.
        _feat_holder = []
        def _hook(module, inp, out):
            _feat_holder.append(out)
        inner_net = model[1]  # unwrap Sequential(Normalize_layer, CifarResNet)
        h = inner_net.avgpool.register_forward_hook(_hook)

        model(perturbed_data)  # run forward to populate _feat_holder
        h.remove()

        # avgpool output shape: [B, 64, 1, 1] -> flatten to [B, 64]
        feat = _feat_holder[0].view(_feat_holder[0].size(0), -1)
        loss = self.criterion(feat[:, tar], target.detach()[:, tar])
...
def validate(val_loader, model, criterion, num_branch):
    ...
    with torch.no_grad():
        for i, (input, target) in enumerate(val_loader):
            target = target.cuda()
            input = input.cuda()
            # [CHANGED #4] Removed volatile=True from Variable() — the volatile argument
            # was removed in PyTorch 1.0 and raises a RuntimeError. Since we are already
            # inside a torch.no_grad() block, this is a no-op replacement.
            target_var = target
            ...
            for output in output_branch:
                # [CHANGED #5] Added dim=1 to suppress UserWarning about implicit dim choice
...
def validate_for_attack(val_loader, model, criterion, num_branch, xh):
    ...
    with torch.no_grad():
        for i, (input, target) in enumerate(val_loader):
            ...
            target = target.cuda()
            input = input.cuda()
            # [CHANGED #4] Same volatile removal as in validate()
            target_var = target
...
# print(index_list)
index_list = [x for x in range(0, 16)]
...
import numpy as np
# [CHANGED #6] Ensure result dir exists before writing
...
# [CHANGED #7] Fixed CUDA index-out-of-bounds crash in trigger target initialisation.
# Original: y = net2(x_var)[15]  ->  shape [1, 10]  (10-class logits)
#           y[:, tar] = high     ->  CRASH: tar values like 56 exceed index 9
#
# Root cause: tar = w_id[targets] gives column indices (0-63) of the weight matrix
# of quan_Linear(64->10). Those index the 64-dim penultimate feature space, NOT
# the 10-class output. Fix: extract the 64-dim avgpool feature via a forward hook.
_feat_init = []
def _hook_init(module, inp, out):
    _feat_init.append(out)
_inner2 = net2[1]  # unwrap Sequential(Normalize_layer, CifarResNet)
_h_init = _inner2.avgpool.register_forward_hook(_hook_init)
net2(x_var)
_h_init.remove()
y = _feat_init[0].view(_feat_init[0].size(0), -1).detach().clone()  # shape [1, 64]
