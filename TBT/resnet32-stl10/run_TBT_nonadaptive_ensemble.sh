#!/bin/bash
#SBATCH --job-name=tbt_stl10_nonadp_ens
...
python -u TBT_nonadaptive_ensemble.py

# CRITICAL: Copy all results back before SLURM wipes TMPDIR
mkdir -p "$SLURM_SUBMIT_DIR/result_stl10_nonadp_ens"
cp -r result/* "$SLURM_SUBMIT_DIR/result_stl10_nonadp_ens/"
echo "=== DONE: Non-Adaptive Ensemble TBT (STL-10) results saved to $SLURM_SUBMIT_DIR/result_stl10_nonadp_ens/ ==="
