...
trainset = torchvision.datasets.CIFAR100(root='../../cifar100/vgg16/data', train=True, download=True, transform=transform_train) 

loader_train = torch.utils.data.DataLoader(trainset, batch_size=128, shuffle=True, num_workers=2) 

testset = torchvision.datasets.CIFAR100(root='../../cifar100/vgg16/data', train=False, download=True, transform=transform_test) 
loader_test = torch.utils.data.DataLoader(testset, batch_size=128, shuffle=False, num_workers=2) 

net_c = models.__dict__['vgg16_quan'](100)
net = torch.nn.Sequential(
                    Normalize_layer(mean,std),
                    net_c
                    )

net_f = models.__dict__['vgg16_quan'](100) 
net1 = torch.nn.Sequential(
                    Normalize_layer(mean,std),
                    net_f
                    )

net_d = models.__dict__['vgg16_quan1'](100)
...
#Loading the weights
pretrain_dict = torch.load('../../cifar100/vgg16/save_finetune/model_best.pth.tar')
...
class Attack(object):
    ...
    def fgsm(self, model, data, target,tar,ep, data_min=0, data_max=1):
        ...
        for i in range(len(output)):
            #if i==4 or i==5 or i==6 or i==7 or i==8 or i==9 or i==10 or i==11 or i==12 or i==13 or i==14:
            tar = T[i]
            loss += self.criterion(output[i][:,tar], target[i].detach()[:,tar])
                #print(loss)
...
for i in range(200):  
    x_tri=model_attack.attack_method(net2, x_var.cuda(), y,T,ep,mins,maxs) 
    x_var=x_tri
...
np.savetxt('./result/vgg_trojan_img1.txt', x_tri[0,0,:,:].cpu().numpy(), fmt='%f')
np.savetxt('./result/vgg_trojan_img2.txt', x_tri[0,1,:,:].cpu().numpy(), fmt='%f')
np.savetxt('./result/vgg_trojan_img3.txt', x_tri[0,2,:,:].cpu().numpy(), fmt='%f')

loader_test = torch.utils.data.DataLoader(testset, batch_size=128, shuffle=False, num_workers=2)
#validate(loader_test, net, criterion, 15)
validate_for_attack(loader_test, net, criterion, 15, x_tri)
...
for param in net.parameters(): 
    ...
    #if n==93 or n==113 or n==133 or n==153 or n==169 or n==179 or n==189 or n==199 or n==209 or n==217 or n==225:
