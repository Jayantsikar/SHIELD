...
We evaluated our defense against Targeted Bit-flip Trojan (TBT) attacks using quantized ResNet-32 models on the \textbf{CIFAR-10} benchmark. (Evaluation on STL-10 is currently ongoing). Table~\ref{tab:cifar10_results} and Table~\ref{tab:stl10_results} summarize the Clean Accuracy (C-Acc) and Attack Success Rate (ASR) across all configurations.

% TODO: INSERT GRAPH/PLOT HERE
% [Indication: Insert a plot showing the Clean Accuracy degradation and ASR mitigation when the defense is active vs. inactive, comparing Baseline Aegis, SBE Undefended, and SBE Defended.]

\begin{table}[!tb]
\caption{TBT Attack vs. Defense on CIFAR-10 (ResNet-32)}
...
\begin{tabular}{@{}llcc@{}}
\toprule
\textbf{Attack} & \textbf{Defense Variant} & \textbf{C-Acc (\%)} $\uparrow$ & \textbf{ASR (\%)} $\downarrow$ \\
\midrule
\multirow{3}{*}{\textbf{Non-Adapt.}} 
    & Baseline (Single Model)              & 88.69 & 17.82 \\
    & \textbf{L1: SBE (Undefended)}        & 90.01 & 15.48 \\
    & \textbf{L2: Self-Heal (Defended)} & \textbf{89.93} & \textbf{11.67} \\
\midrule
\multirow{3}{*}{\textbf{Adaptive}}   
    & Baseline (Single Model)              & 83.41 & 38.50 \\
    & \textbf{L1: SBE (Undefended)}        & 87.39 & 40.72 \\
    & \textbf{L2: Self-Heal (Defended)} & \textbf{83.02} & \textbf{15.09} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[!tb]
\caption{TBT Attack vs. Defense on STL-10 (ResNet-32)}
...
\midrule
\multirow{3}{*}{\textbf{Non-Adapt.}} 
    & Baseline Aegis              & TBD & TBD \\
    & \textbf{L1: SBE (Undefended)}        & TBD & TBD \\
    & \textbf{L2: Self-Heal (Defended)} & \textbf{TBD} & \textbf{TBD} \\
\midrule
\multirow{3}{*}{\textbf{Adaptive}}   
    & Baseline Aegis              & TBD & TBD \\
    & \textbf{L1: SBE (Undefended)}        & TBD & TBD \\
...
\begin{itemize}
    \item \textbf{Evaluation Ongoing:} Results for the STL-10 dataset are currently pending and will be updated in future revisions.
\end{itemize}
