...
trainset = torchvision.datasets.CIFAR100(root='../../cifar100/resnet32/data', train=True, download=True, transform=transform_train) 
...
net = models.__dict__['resnet32_quan1'](100)
net1 = models.__dict__['resnet32_quan'](100)
pretrain_dict = torch.load('../../cifar100/resnet32/save_finetune/model_best.pth.tar')
...
def find_psens(model, data_loader, perturbed):
    ...
    loss_mse = criterion1(output_nolast,y)
...
def find_optim_value(model, psens, ele_loc, data_loader, choice, perturbed, num):
    ...
    output_nolast = model(data,nolast=True)[15]
    output1 = model(data)[15]
    loss_cbs = criterion1(output_nolast,y) + criterion2(output1,ys_target)
...
def validate(val_loader, model, criterion, num_branch):
    ...
    with torch.no_grad():
        for i, (input, target) in enumerate(val_loader):
            ...
            num_c = 9#6 # the number of branches 
...
print(psens)
loader_test = torch.utils.data.DataLoader(testset, batch_size=16, shuffle=False, num_workers=2)
num=0
last_loc=0
