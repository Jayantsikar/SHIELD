...
parser.add_argument('--weight', default='1, 1, 1, 1, 1, 1, 1', 
                      help='weight')
...
def main():
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            checkpoint = torch.load('/home/wangjialai/copy_for_use/flip_attack/BFA_fineunet_branch/save/cifar10_resnet20_quan_200_SGD_binarized/checkpoint_branch.pth.tar')
        ...
        checkpoint = torch.load("/home/wangjialai/copy_for_use/flip_attack/BFA/save/cifar10_resnet20_quan_160_SGD_binarized/model_best.pth.tar")
        state_tmp = net.state_dict()
        if 'state_dict' in checkpoint.keys():
            state_tmp.update(checkpoint['state_dict'])
        else:
...
def perform_attack(attacker, model, model_clean, train_loader, test_loader,
    ...
    for i_iter in range(N_iter):
        ...
        val_acc_top1, val_acc_top5, val_loss= validate_for_attack(
            test_loader, model, attacker.criterion, log, T)
        # tmp_df = pd.DataFrame(output_summary, columns=['top-1 output'])
...
def train(train_loader, model, criterion, optimizer, epoch, log, list_shape, flip_highest):
        ...
        if flip_highest:
            for name, m in model.named_modules():
                if isinstance(m, quan_Conv2d) or isinstance(m, quan_Linear):
                    ...
                    #if m.weight.grad is not None:
                    if ('conv_1' in name) or ('stage_1' in name):
                        cur_shape = list_shape[count]
                        count+=1
                        
                        tmp=m.weight.detach().clone()
                        tmp_n=tmp.cpu().detach().numpy()

                        tmp_n= np.round(tmp_n / m.step_size.cpu().detach().numpy()) # calcualte the minimum weight value
                        tmp_uint8 = tmp_n.astype("int8")
                        #print("tmp_uint8:", tmp_uint8[-1])
                        #tmp_uint8[cur_shape==1] = tmp_uint8[cur_shape==1] ^ 0x80
                        n_xor=np.int8(-128)
                        #n_xor=n_xor.astype("int8")
                        tmp_uint8 = tmp_uint8 ^ n_xor#0x80
                        
                        tmp_uint8 = tmp_uint8.astype('float32') * m.step_size.cpu().detach().numpy()

                        m.weight.data = torch.tensor(tmp_uint8.astype('float32')).cuda()
                        #m.__reset_stepsize__()
        ...
        for name, m in model.named_modules():
            if isinstance(m, quan_Conv2d) or isinstance(m, quan_Linear):
                if ('conv_1' in name) or ('stage_1' in name):
                    #cur_shape = list_shape[count]
                    count+=1
                    #print("w grad:", m.weight.grad)
                    tmp=m.weight.detach().clone()
                    tmp_n=tmp.cpu().detach().numpy()
                    min_v= min_pos * m.step_size.cpu().detach().numpy() # calcualte the minimum weight value
                    # tmp_n[(tmp_n>=-min_v) & (tmp_n<=0)&(cur_shape==1).numpy()] = -min_v
                    # tmp_n[(tmp_n<=min_v) & (tmp_n>=0)&(cur_shape==1).numpy()] = min_v
                    
                    tmp_n[(tmp_n>=-min_v) & (tmp_n<=0)] = -min_v
                    tmp_n[(tmp_n<=min_v) & (tmp_n>=0)] = min_v
    ...
    print_log(
        ...
        .format(top1_b1=top1_list[0], top5_b1=top5_list[0], error1=100 - top1_list[0].avg,
                ...
                top1_bmain=top1_list[6], top5_bmain=top5_list[6], errormain=100 - top1_list[6].avg,
        ), log)
...
def validate(val_loader, model, criterion, log, summary_output=False):
    ...
    with torch.no_grad():
        ...
        .format(top1_b1=top1_list[0], top5_b1=top5_list[0], error1=100 - top1_list[0].avg,
                ...
                top1_b5=top1_list[4], top5_b5=top5_list[4], error5=100 - top1_list[4].avg,
                top1_b6=top1_list[5], top5_b6=top5_list[5], error6=100 - top1_list[5].avg,
                top1_main=top1_list[6], top5_main=top5_list[6], errormain=100 - top1_list[6].avg,
        ), log)
...
def validate_for_attack(val_loader, model, criterion, log, T):
    losses = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
    ...
    top1_list = []
    for idx in range(7):# acc list for all branches
        top1_list.append(AverageMeter())
    top5_list = []
    for idx in range(7):
        top5_list.append(AverageMeter())
    ...
    with torch.no_grad():
        for i, (input, target) in enumerate(val_loader):
            ...
            #res = vote_for_predict(output_branch, target_var.unsqueeze(axis=1), topk=(1,))
            ...
            for j in range(input.size(0)):
                ...
                #print("T5:", T[5])
                #sys.exit()
                #if (measure_branch6.data).cpu().numpy()[j] * ( 1 + np.random.uniform(low=-r, high=r))< T[5] :
                #if (measure_branch6.data).cpu().numpy()[j] * ( 1 + np.random.uniform(low=-r, high=r))< 0.34 :
                #print(" pro: ", prob_branch6.data.cpu().numpy()[j])
                #print("out pro:", prob_main.data.cpu().numpy()[j])
                #sys.exit()

                for item in range(len(output_branch)):# # calculate acc for all branches
                    out = Variable(torch.from_numpy(output_branch[item].data.cpu().numpy()[j].reshape((1,-1))).float().cuda())
                    prec1, = accuracy(out.data, tar, topk=(1,))
                    top_list[item].update(prec1, 1)
                    
                #if max(prob_main.data.cpu().numpy()[j]) > 0.95:
                #if (measure_main.data).cpu().numpy()[j] * ( 1 + np.random.uniform(low=-r, high=r))< T[6]-0.12 :
                    #print(prob_branch4.data.cpu().numpy()[j], prob_branch5.data.cpu().numpy()[j], prob_main.data.cpu().numpy()[j])
                #if False:
                #if False:
                p = random.random()
                #if False:
                if p>0.5:
                    ...
                    decision.append(6)
                    count_list[6]+=1
                elif (measure_branch1.data).cpu().numpy()[j] * ( 1 + np.random.uniform(low=-r, high=r)) < T[0] - 0.5:
                #if False:
                #elif False:
                ...
                elif (measure_branch2.data).cpu().numpy()[j] * ( 1 + np.random.uniform(low=-r, high=r)) < T[1]-0.8: 
                #elif False: 
                #elif max(prob_branch2.data.cpu().numpy()[j]) > 0.2:
