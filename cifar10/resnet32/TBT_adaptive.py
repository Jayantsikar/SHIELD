...
class Attack(object):
    ...
    def fgsm(self, model, data, target,tar,ep, data_min=0, data_max=1):
        ...
        for i in range(len(output)):
            if i>5 and i!=7:
                tar = T[i]
                tar_idx = int(tar.item()) % output[i].shape[1]  # clamp to valid column range
                loss += self.criterion(output[i][:,tar_idx], target[i].detach()[:,tar_idx])
        #print(loss)
    ...
    with torch.no_grad():
        for i, (input, target) in enumerate(val_loader):
            ...
            for j in range(input.size(0)):
                ...
                _pool = index_list[4:] if len(index_list[4:]) >= num_c else (index_list if len(index_list) >= num_c else list(range(16)))
                pre_index = random.sample(_pool, num_c)
                c_ = 0
...
os.makedirs('./result/TBT', exist_ok=True)
...
for i in range(len(y)):
    if i>5 and i!=7:
        tar = T[i]
        tar_idx = int(tar.item()) % y[i].shape[1]  # clamp to valid column range
        y[i][:,tar_idx]=high   ### setting the target of certain neurons to a larger value 10
