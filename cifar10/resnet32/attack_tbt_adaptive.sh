...
#SBATCH --mem=32G
#SBATCH --output=%j_tbt_adaptive.out
#SBATCH --error=%j_tbt_adaptive.err

echo "============================================================"
echo " TBT Adaptive Attack — ResNet32 / CIFAR-10"
...
if [ ! -f "./save_finetune/model_best.pth.tar" ]; then
    ...
    if [ -n "$LATEST" ] && [ -f "${LATEST}save_finetune/model_best.pth.tar" ]; then
        ...
        echo "Please run train_gpu.sh first."
        exit 1
    fi
fi
...
echo "[3/3] Running TBT adaptive attack..."
python TBT_adaptive.py

EXIT_CODE=$?

mkdir -p ~/job_results/$SLURM_JOB_ID
mkdir -p ~/Aegis/results/cifar10/resnet32/tbt_adaptive
cp -r ./ ~/job_results/$SLURM_JOB_ID/
cp ${SLURM_JOB_ID}_tbt_adaptive.out ~/Aegis/results/cifar10/resnet32/tbt_adaptive/ 2>/dev/null
cp ${SLURM_JOB_ID}_tbt_adaptive.err ~/Aegis/results/cifar10/resnet32/tbt_adaptive/ 2>/dev/null
