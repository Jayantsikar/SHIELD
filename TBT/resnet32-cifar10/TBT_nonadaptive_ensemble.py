...
# Each IC gets an independent random seed so honey channels differ.
# ============================================================
os.makedirs('./result', exist_ok=True)
_ics = [net, net1, net2]       # all three ICs used in this script
# _neuropots_defenses = []
# for _i, _ic in enumerate(_ics):
#     _cfg = TrapdoorConfig(
#         conv_layer_indices=[0, 2, 4],   # first 3 quan_Conv2d layers
#         honey_channels_per_layer=2,
#         gamma=2.0,
#         tolerance=0.05,
#         max_strikes=3,
#         seed=42 + _i * 7,              # unique seed per IC
#     )
#     _defense = NeuroPotsDefense(model=_ic, config=_cfg, ic_index=_i)
#     _defense.embed_one_shot_trapdoors()  # in-place weight edit, no grad
#     _defense.save_registry(f'./result/ic{_i}_honeypot_nonadaptive.pt')
#     _neuropots_defenses.append(_defense)
# 
# # Trust registry: 16 branches but only 3 underlying IC models.
# # The registry tracks trust at the IC (model) level; all branches
# # belonging to a compromised IC get zeroed together via trust_mask.
# # Here we use a 16-slot registry where slots 0..15 map to net (slot 0
# # is the only IC in this non-adaptive script — the single `net` object
# # produces all 16 branches, so we track it as IC-0 only).
# _neuropots_registry = ICTrustRegistry(
#     num_ics=1,              # one IC (net) produces all 16 branches
#     max_strikes=3,
...
class Attack(object):
    ...
    def fgsm(self, model, data, target,tar,ep, data_min=0, data_max=1):
        ...
        # [CHANGED] Use avgpool forward hook to get the 64-dim penultimate feature vector
        _feat_holder = []
...
def validate2(val_loader, model, criterion, num_branch):
    ...
    # Sort all branch indices by accuracy (ascending: worst→best)
    # index_list[4:] will always have ≥12 entries for random.sample(..., 3)
    branch_accs = [(top1_list[i].avg, i) for i in range(len(top1_list))]
    branch_accs.sort(key=lambda x: x[0])  # ascending: index_list[4:] = top performers
...
def validate(val_loader, model, criterion, num_branch):
    ...
    with torch.no_grad():
        for i, (input, target) in enumerate(val_loader):
            ...
            # net1/net2 are clean reference copies for weight restoration, not monitored.
            ...
            all_probs = torch.stack([item[0] for item in out_list], dim=1) # [batch, num_branch, 10]
            all_weights = torch.stack([item[1] for item in out_list], dim=1) # [batch, num_branch]
...
def validate_clean_defended(val_loader, model, criterion, num_branch):
    ...
    is detected — expected to stay near the original ~90%."""
    losses = AverageMeter()
    top1 = AverageMeter()
    count_list = [0] * num_branch
...
for epoch in range(200): 
    ...
    if (epoch+1)%50==0:     
        ...
        torch.save(net.state_dict(), './result/final_trojan_nonadaptive_ensemble.pkl') ## saving the trojaned model
        ...
        torch.save(net, './result/final_trojan_nonadaptive_ensemble.pth')
...
print('Model saved to ./result/final_trojan_nonadaptive_ensemble.pth')

print('\n============================================================')
print('  SUMMARY — Non-Adaptive Ensemble')
print('============================================================')
...
os.makedirs('./result', exist_ok=True)
with open('./result/final_results_nonadaptive_ensemble.txt', 'w') as f:
    f.write(f'TBT Non-Adaptive Ensemble Attack Results\n')
    f.write(f'========================================\n')
